...
# Transformer-aware fork of run_opf.jl for network-expansion campaigns.
# Identical to run_opf.jl EXCEPT the branch loop honors optional per-branch
# `tap`, `shift`, and `transformer` fields (defaulting to a nominal 1.0 / 0.0 /
# false), so expansion plans that insert off-nominal-tap or phase-shifting
# transformers are modeled correctly. The production run_opf.jl is left
# untouched so live campaigns are unaffected.

if length(ARGS) < 3
    error("usage: run_opf_expansion.jl <case_json> <payload_json> <out_json>")
end
...
function to_powermodels_data(case_data)
    ...
    for (idx, br) in enumerate(branches)
        ...
        # Optional transformer fields: off-nominal tap ratio and phase shift.
        tap = haskey(br, :tap) ? Float64(br[:tap]) : 1.0
        shift = haskey(br, :shift) ? Float64(br[:shift]) : 0.0
        is_xfmr = if haskey(br, :transformer)
            Bool(br[:transformer])
        else
            tap != 1.0 || shift != 0.0
        end
...
if task == "ac_opf"
    ...
    result["note"] = "Only ac_opf task is currently implemented in run_opf_expansion.jl"
end

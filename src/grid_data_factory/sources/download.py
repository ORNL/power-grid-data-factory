...
def destination_dir(spec: dict) -> str:
    return str(spec.get("destination") or spec.get("target_dir"))


def source_url(spec: dict) -> str | None:
...
def git_head(repo_dir: Path) -> str | None:
    if not (repo_dir / ".git").exists():
        return None
    proc = run_command(["git", "rev-parse", "HEAD"], repo_dir)
    if proc.returncode != 0:
        return None
    return proc.stdout.strip() or None


def download_with_curl(url: str, out_file: Path, repo_root: Path) -> tuple[bool, str]:
...
def normalize_legacy_archive_location(target_dir: Path, filename: str, raw_dir: Path) -> Path:
...
def extract_archive(archive_path: Path, extract_root: Path) -> tuple[bool, str | None, str | None]:
...
def prepare_manual_catalog(spec: dict, target_dir: Path) -> None:
...
def process_git_source(repo_root: Path, spec: dict, target_dir: Path, args: Any, source_report: dict, report: dict) -> None:
    repo_url = source_url(spec)
    if not repo_url:
    ...
    if target_dir.exists() and pinned and args.checkout_pinned:
        proc = run_command(["git", "checkout", str(pinned)], target_dir)
        if proc.returncode == 0:
    ...
    source_report["git_head"] = git_head(target_dir) if target_dir.exists() else None
    if pinned:
        source_report["pinned_commit"] = str(pinned)
        source_report["pinned_match"] = source_report["git_head"] == str(pinned)


def iter_download_specs(spec: dict) -> list[dict]:
    entries: list[dict] = []
...
def process_archive_source(
    ...
    downloads = iter_download_specs(spec)
    ...
    for item in downloads:
        ...
        out_file = normalize_legacy_archive_location(target_dir, filename, raw_dir)
        ...
        if args.download_files and not already_verified:
            if not out_file.exists() or (expected_sha and not file_meta.get("sha256_match", False)):
                ok, err = download_with_curl(url, out_file, repo_root)
                if ok:
            ...
            if out_file.exists():
                ...
                file_meta["sha256"] = sha
                file_meta["size_bytes"] = out_file.stat().st_size
                file_meta["downloaded_at"] = utc_now_iso()
                if expected_sha:
                    file_meta["expected_sha256"] = str(expected_sha)
                    file_meta["sha256_match"] = sha == str(expected_sha)
        ...
        if out_file.exists() and args.extract_archives:
            ok, extracted_path, mode = extract_archive(out_file, extract_dir)
            if ok:
    ...
    write_yaml(manifest_path, source_manifest)
    source_report["source_manifest"] = str(manifest_path)
    source_report["downloads"] = archives_meta


def check_required_cases(spec: dict, require_cases: set[str], source_report: dict, report: dict) -> None:

...
def _resolve_case_ids_from_config(repo_root: Path, source: str) -> list[str]:
    if yaml is None:
        raise RuntimeError("PyYAML is required for --all-config-cases")
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Run AC-OPF via pandapower on one or more MATPOWER cases.")
    ...
    p.add_argument("--solver-id", default="pandapower_ac_opf")
...
def _write_common_attempt_files(in_progress: Path, run_meta: dict[str, Any], cmd_args: list[str]) -> None:
    ...
    (in_progress / "command.txt").write_text("python scripts/run_pandapower_ac_opf.py " + " ".join(cmd_args), encoding="utf-8")
    (in_progress / "command.json").write_text(
        json.dumps({"executable": "python3.11", "args": ["scripts/run_pandapower_ac_opf.py", *cmd_args]}, indent=2),
        encoding="utf-8",
    )
    (in_progress / "environment" / "environment.json").write_text(
        json.dumps({"created_at": _now(), "python": "3.11", "script": "run_pandapower_ac_opf.py"}, indent=2),
        encoding="utf-8",
    )
...
def _run_one_case(repo_root: Path, args: argparse.Namespace, case_id: str, case_file: Path) -> dict[str, Any]:
    ...
    (in_progress / "inputs" / "resolved_case.json").write_text(
        json.dumps({"case_id": case_id, "case_file": str(case_file)}, indent=2),
        encoding="utf-8",
    )
    ...
    (in_progress / "logs" / "stderr.log").write_text(str(result.get("stderr", result.get("error", ""))), encoding="utf-8")
...
def main() -> None:
    ...
    for case_id, case_file in plan:
        ...
        try:
            out = _run_one_case(repo_root, args, case_id, case_file)
            results.append(out)
